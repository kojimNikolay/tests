...
class Model_Todo extends Model
{
  ...
  public function paginatorData($count, int $from, $sort_by)
  {
    $data = []; //, name DESC
	$tongle = $_SESSION['tongle'];
	
	
    
  switch ($sort_by) {
    case 1:
        if($tongle) $query = "SELECT * FROM todos ORDER BY name ASC ";
        else $query = "SELECT * FROM todos ORDER BY name DESC ";
        break;
    case 2:
        if($tongle) $query = "SELECT * FROM todos ORDER BY email ASC ";
        else $query = "SELECT * FROM todos ORDER BY email DESC ";
        break;
    case 3:
        if($tongle) $query = "SELECT * FROM todos ORDER BY text ASC ";
        else $query = "SELECT * FROM todos ORDER BY text DESC ";
        break;
    case 4:
      if($tongle) $query = "SELECT * FROM todos ORDER BY status ASC ";
      else $query = "SELECT * FROM todos ORDER BY status DESC ";
      break;
